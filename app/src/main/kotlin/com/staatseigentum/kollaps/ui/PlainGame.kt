 ...
 *
 * Everything that is not Android runs on this: the PC window, the screenshot harness and the
 * iPhone. Android has its own driver because it has a lifecycle to answer to — a view model that
 * survives a rotation and a coroutine scope that ends with the screen. Nothing else does, and
 * writing that driver a second and a third time would have been three copies of forty
 * one-line overrides that must not drift apart.
 *
 * Holds no save file of its own. Each platform's entry point loads one and writes it back; the
 ...
 *
 * Open because the platform that owns it may want to add to it, not because anything here is
 * meant to be replaced — every method is the same one-line call into [GameEngine] on all of them.
 */
open class PlainGame(start: GameState = GameState.new(NOW)) : GameActions {
     ...
     */
    fun settleWallClock() {
        state = GameEngine.onWallClock(state, Wallclock.millis())
    }
    ...
    fun load(loaded: GameState?) {
        state = loaded ?: GameState.new(Wallclock.millis())
    ...
    fun creditTimeAway() {
        val report = GameEngine.applyOffline(state, Wallclock.millis())
        state = report.state
    ...
    override fun startResearch(id: String) {
        state = GameEngine.startResearch(state, id, Wallclock.millis())
    }
    ...
    override fun developGalaxy(slot: Int) {
